...
class OAuthController(http.Controller):
    ...
    def add_access_token(self, **kw):
        if kw.get('access_token'):
            params = {
                'client_id': request.env['ir.config_parameter'].sudo().get_param('meta_crm_facebook_leads.crm_fb_app_id'),
                'client_secret': request.env['ir.config_parameter'].sudo().get_param(
                    'meta_crm_facebook_leads.crm_fb_app_secret'),
            ...
            request.env['ir.config_parameter'].set_param("meta_crm_facebook_leads.crm_fb_access_token",
                                                         r.get('access_token', ''))
