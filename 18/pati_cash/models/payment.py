...
class CustomPayment(models.Model):
    ...
    def _compute_show_customer(self):
        for rec in self:
            rec.show_customer = rec.debit_account_id and rec.debit_account_id.code == '101001'

    @api.depends()
    def _compute_can_approve(self):
        """Check if current user is in Custom Payment Approver group."""
        user = self.env.user
        has_group = user.has_group('custom_payment.group_custom_payment_approver')
        for rec in self:
            rec.can_approve = has_group

    @api.onchange('company_id')
    def _onchange_company_id(self):
        """Update domain for journal, debit and credit accounts safely."""
        for rec in self:
            # Journal: filter by company_id (exists in account.journal)
            journal_domain = [('name', 'ilike', 'Miscellaneous Operations')]
            if rec.company_id:
                journal_domain.append(('company_id', '=', rec.company_id.id))
            ...
            # Domain for debit accounts (account.account has no company_id)
            debit_domain = [('code', 'in', ['101401', '101405', '101406'])]
            credit_domain = [('name', 'ilike', 'Cash')]

            # Set domains
            domain = {
                'debit_account_id': debit_domain,
                'credit_account_id': credit_domain,
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            ...
            if not sequence:
                ...
                _logger.info("Created sequence for company %s", company_id)
            ...
            try:
                ...
                _logger.warning(
                    "Fallback sequence used for payment %s (company %s). Error: %s",
                    vals['name'], company_id, e
                )
    ...
    def action_approve_payment(self):
        ...
        try:
            ...
            self.status = 'done'
            _logger.info("Journal entry %s created for payment %s", move.name, self.name)
        except Exception as e:
            _logger.error("Failed to create journal entry for payment %s: %s", self.name, e)
