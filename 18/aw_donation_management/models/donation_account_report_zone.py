...
class DonationAccountReportZone(models.Model):
    _name = 'donation.account.report.zone'
    _description = 'Donation Account Report Zone'
    ...
    multan_sharif_construction = fields.Float(string='Multan Sharif Construction', digits=(16, 2), default=0.0)
    ...
    lahore_fund = fields.Float(string='Lahore Fund', digits=(16, 2), default=0.0)
    ...
    @api.depends(
        ...
        'multan_sharif_construction', 'sadqa_mulaqat', 'multan_sharif_fund', 
        'naat_khaani', 'qurbani_wala_mamla', 'hadia', 'sadqa_shifa', 
        'special_fund_lahore', 'langer_shareef_multan', 'langer_shareef', 
        'syedi_g_ki_neeyat_se_sadqa', 'hadia_aqa_ji', 'faisal_town_purchase_fund',
        'qurbani_zati', 'qurbani_sarkari', 'hadia_saad_bin_waqas', 
        'hadia_ameer_hamza', 'lahore_fund'
    )
    def _compute_total_amount(self):
        ...
        for record in self:
            record.total_amount = sum([
                ...
                record.hadiya_imam_hussain, record.multan_sharif_construction,
                record.sadqa_mulaqat, record.multan_sharif_fund, record.naat_khaani,
    ...
    def _compute_display_fields(self):
        ...
        for record in self:
            ...
            record.display_name = f"{company_name} - {record.period}" if company_name and record.period else 'Donation Report Zone'

    def name_get(self):
        """Custom name display"""
        return [(record.id, record.display_name or 'Donation Report Zone') for record in self]
        ...
        if records and self.env.context.get('show_totals', False):
            ...
            if company_filter or self.env.context.get('force_show_totals', False):
                total_row = {field: 0.0 for field in fields if field in [
                    ...
                    'multan_sharif_construction', 'sadqa_mulaqat', 'multan_sharif_fund',
                    'naat_khaani', 'hadia', 'sadqa_shifa', 'special_fund_lahore',
                    'qurbani_wala_mamla', 'langer_shareef_multan', 'langer_shareef',
                    'syedi_g_ki_neeyat_se_sadqa', 'hadia_aqa_ji', 'faisal_town_purchase_fund',
                    'qurbani_zati', 'qurbani_sarkari', 'hadia_saad_bin_waqas',
                    'hadia_ameer_hamza', 'lahore_fund', 'total_amount'
                ]}
    ...
    def get_company_summary(self, company_id=None, month=None, year=None):
        ...
        summary = {
            ...
            'multan_sharif_construction': sum(records.mapped('multan_sharif_construction')),
            ...
            'lahore_fund': sum(records.mapped('lahore_fund')),
        ...
        for record in self:
            ...
            if existing:
                raise models.ValidationError(
                    f"A zone report already exists for {record.company_id.name} - "
